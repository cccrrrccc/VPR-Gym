...
e_create_move MedianMoveGenerator::propose_move_with_type(t_pl_blocks_to_be_moved& blocks_affected, e_move_type& /*move_type*/, float rlim, const t_placer_opts& placer_opts, const PlacerCriticalities* /*criticalities*/, const char* blk_type_name) {
    auto& place_ctx = g_vpr_ctx.placement();
    auto& cluster_ctx = g_vpr_ctx.clustering();
    auto& device_ctx = g_vpr_ctx.device();
    ...
    /* Pick a random block to be swapped with another random block.   */
    ClusterBlockId b_from;
    b_from = pick_from_block_type(blk_type_name);
